...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST)) {
        ...
        if (!empty($_POST['fone'])) {
            ...
            $foneErro = 'Por favor digite o número do telefone!';
            $validacao = False;
        }
        ...
        if (!empty($_POST['nomeconjuge'])) {
            $nomeconjuge = $_POST['nomeconjuge'];
        } else {
            $nomeconjugeErro = 'Por favor selecione um campo!';
            $validacao = False;
        }
    }
    ...
    if ($validacao) {
        ...
        $sql = "INSERT INTO tb_alunos (nome, endereco, fone, email, idade, nomeconjuge)
VALUES(?,?,?,?,?,?)";
        $q = $pdo->prepare($sql);
        $q->execute(array($nome, $endereco , $fone, $email, $idade, $nomeconjuge));
        Banco::desconectar();
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form class="form-horizontal" action="create.php" method="post">
                        ...
                        <div class="control-group <?php echo !empty($foneErro) ? 'error ' : ''; ?>">
                            ...
                            <div class="controls">
                                ...
                                <input size="35" class="form-control" name="fone" type="text"
                                    ...
                                    value="<?php echo !empty($fone) ? $fone : ''; ?>">
                        ...
                        <div class="control-group <?php echo !empty($nomeconjugeErro) ? 'error ' : ''; ?>">
                            <label class="control-label">Nome Conjuge</label>
                            <div class="controls">
                                <input size="50" class="form-control" name="nomeconjuge" type="text"
                                    placeholder="NomeConjuge"
                                    value="<?php echo !empty($nomeconjuge) ? $nomeconjuge : ''; ?>">
                                <?php if (!empty($nomeconjugeErro)): ?>
                                    <span class="text-danger"><?php echo $nomeconjugeErro; ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
